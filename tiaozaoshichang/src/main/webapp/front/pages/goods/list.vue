<template>
	...
	<button
		...
		v-if="isAuth('goods','新增')" class="add-btn" @click="onAddTap()">发布商品</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			// 获取用户信息
			let resA = await this.$api.session(table);
			this.user = resA.data;
			this.btnColor = this.btnColor.sort(() => {
				return (0.5 - Math.random());
			});
			let res = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode:'goods_types'
			});
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (indexQueryCondition) {
					params['goodsName'] = indexQueryCondition;
